...
contract xyz_com{
    ...
    bool __product_defective = false;
 ...
 //for example we cannot give the seller the authority to verify if the product is delivered only the customer can do that
   ...
   function isProductDefective(bool product_defective) public notOwner notSeller{
       //only customer has authority to call this function, since only customer can tell if they received a defective product or not
    __product_defective = product_defective;
   }
    function refund() public payable onlyOwner{
        //the owner will refund the money to the customer if they haven't received the product or received a defective one  
        require(product_delivered==false || __product_defective == true,"Product is delivered successfully and in good condition");
    ...
    function transferFunds() public onlyOwner{
        ...
        require(__product_defective== false);
